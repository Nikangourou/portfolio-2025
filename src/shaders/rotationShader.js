...
export const rotationVertexShader = `
  ...
  void main()
  {
      ...
      // Direction du vent selon la rotation
      float windDirection = uRotationDirection;
      
      // Ondulations de base
      float baseElevation = sin(modelPosition.x * uFrequency.x - uTime) * 0.1 * totalIntensity;
      baseElevation += sin(modelPosition.y * uFrequency.y - uTime) * 0.1 * totalIntensity;
      
      // Déformation par résistance à l'air (toute la surface réagit)
      float airDeformation = sin(modelPosition.x * uFrequency.x * 1.5 + uTime * 2.0 * windDirection) 
                           * 0.06 * airResistance;
      airDeformation += sin(modelPosition.y * uFrequency.y * 1.2 - uTime * 1.5 * windDirection) 
                       * 0.05 * airResistance;
      
      // Effet de turbulence (plus complexe avec la vitesse)
      float turbulence = sin(modelPosition.x * uFrequency.x * 2.0 + modelPosition.y * uFrequency.y + uTime * 3.0 * windDirection) 
